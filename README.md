...
- run the programs generated by each compiler
...
![Tends-Crash Rate](https://raw.githubusercontent.com/Groostav/CMPT886-Paper-1/master/compiler_crash_result.jpg)

This shows the crash rate on GCC and LLVM with repsect to tests generated by CSmith, 

 It's pretty good news, aside from a spike with the 4.0 release of GCC, its trending down.

---

# Trends (cont) #

![Trends-Wrong code](https://raw.githubusercontent.com/Groostav/CMPT886-Paper-1/master/compiler_wrong_crash.jpg)

This shows the number of wrong-code instances generated by the compiler here

 Again, _reasonably_ good news, Recently both LLVm and GCC have been trending down, if we forgive GCC < 4.0 and LLVM < 2.4.

---

# Trends (cont 2) #

Paper also included metrics for internal consistency checks
 
 - _Read: lines inside the compilers like this:_
 ```
 if(someExpressionNode.importantDetail == nullptr){
   throw "Important detail not yet set!";
 }
 ```

 - We omitted them because they didnt really suggest anything without further research. 
 
  - The number of failures from consistency-check problems could simply be the result of adding/removing/modifying those checks inside the compilers
...
So the only formally verified compiler they tested performed head-and-shoulders above all the other compilers, including GCC, LLVM, and the Intel Compiler.

 - It also doesn't support a number of language features that these other compilers support
...
- Generates a reasonably human-friendly item for testing (source code) --unlike AST or IL generation schemes.

**Cons:**

- tests are not minimal or concise -> not easy to read.

- poor localization on bugs: given a problem, you dont know if its in the front, middle, or back end. 
